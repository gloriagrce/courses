...
1. What happens if you make the number of conjectured values of $p$ (the fine-ness of the grid) smaller, or larger? 
...
Re-run your code a few times to see what happens with very different grid resolutions.

2. (*relies on previous R experience*) What if you use a different prior - how do results change? If I use a different prior, 
3. (*advanced/tricky*) Why are the posterior, the prior and the likelihood in our plot different heights? Because for prior since it is determined as 1, it's different from the others. For posterior, since it is the probability of model being right given the probability of data, it have different height with the likelihood. 

Why doesn't that bother Prof DR a lot?

4. What happens if you use a smaller dataset (just the initial observations instead of all of them)? The probability increase.  A larger one? The probability would decrease.
